...
- 當使用者貼一個 `model_check/<...>.iges` 或 `IGES/<...>.iges` 路徑時，**Claude 必須從路徑識別這是哪個物理模擬模型**（Long Fei 半切六極 / Kuo Quadrupole / Zhang Quadrupole / ...），把該模型當作後續對話的「當前討論模型」，**不要問「這是哪個模型？」**；模型清單見 `magnetic_sim/.claude/rules/iges-model-id.md`
