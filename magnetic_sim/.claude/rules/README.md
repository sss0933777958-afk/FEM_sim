...
- `calibration-transfer-matrix-output.md` — current_base / voltage_base 結果 PDF 除 K̄_I / D̄（無因次），一律加印 gauge 前物理轉移矩陣 ᴮH̄_I [mT/A] = ĝ_I·K̄_I 與 ᴮH̄_V [mT/mV] = ĝ_V·D̄；**附則一（2026-08-04 改）**：USE_BIAS 時偏移**只印無因次 `e/ℓ̂` 一張**（不再印 µm 版）。**附則二（2026-08-05）**：K̄(2,2) 固定 0.8340（只改 `G(2,2)`、ℓ̂/e/其餘元素不動）→ **`voltage_base` 的 G 必須與 `current_base` 完全相同**、兩邊 RMSPE 必一致（0.3381%）；檔名 tag `k22_0p8340`、不覆蓋自由擬合版；揭露寫在 `matlab/Maxwell/results/README.md`（PDF 內不印）。**附則三（2026-08-15）**：擬合誤差 PDF 一律印 **NMAE**（`Σ|ε|/Σ|b|·100`），**RMSPE 停用**（只留 `.mat` 追溯）；四支 solve + 兩支 emit + 兩支 main 兩分支都要同步；另 sensor 距非 4.572mm 時 soff tag 要併進 `VAR_OUT`（否則 PDF 被覆蓋）。
