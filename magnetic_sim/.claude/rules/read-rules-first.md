...
> **2026-07-06 更新**：原 `main/rules/` 6 條規則已移到本層 `…/FEM_sim/.claude/rules/`、**改為每 session 自動載入**，所以「先讀」已由系統自動完成——本檔保留為提醒 + 清單。以下文中「`main/rules/`」現指本 `.claude/rules/` 層。

**使用者拍板（2026-06-26）**：在 `magnetic_sim/ANSYS/main/` 下動手做**任何**工作之前，全部規則須已在 context（移入 `.claude/rules/` 後已自動載入），動到對應路徑前讀該規則全文。
...
| `figure-style.md`（繪圖風格） | 畫任何圖之前（**先問使用者要哪個風格選項**；3D 框體＝兩變體：A 同尺度立方＝手動 `draw_box_edges`(省最遠角3邊)+box off+`daspect`+三軸同刻度、B 異質軸(surf 山丘)＝`box on`+`pbaspect`（判準＝能否 daspect、A/B 不可混用）；數值標註 10^0/無單位不標） |
| `figure-output.md`（繪圖輸出） | 畫任何圖（**輸出實檔→覆蓋迭代到定案**） |
| `actuator-frame.md`（座標系） | 在 `Calibration_using_FEM_modeling/` 下寫/改 load/fit/主程式（**一律 actuator frame**） |
