...
%     故 build_actuator_data 的 zoff = −SPH_OFST 照樣成立。
    ...
    % [ADDED 2026-09-04] maxwell_test = 2026-09-04 09:08~09:58 重匯的一組（同框同步距）。
    %   ⚠ B_p4_test.fld 的匯出框 z 下界是 -13.11mm（不是其餘五顆的 -13.31mm）→ 61x61x51
    %     = 189,771 點，比其餘五顆少 37,210 點。extract_maxwell_data 的「六顆同格」檢查
    %     會擋掉它。WP 中心 z=-12.7113mm，故 r<=400um 的球完全在 P4 框內、校正不受影響，
    %     但要跑整組得先把六顆裁到共同格（或重匯 P4）。
