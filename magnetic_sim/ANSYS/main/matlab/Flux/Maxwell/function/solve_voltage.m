function [D_bar, gV_hat, G, rm] = solve_voltage(l_hat, e, Pc_base, P, Bstack, V, K22_set, Peval, Beval, MODEL)
%SOLVE_VOLTAGE  電壓側解參數：優化器解完 → 重建 S/Pc_bar → G → D̄ / ĝ_V / 𝒞κ。
%   [D_bar, gV_hat, G, rm] = SOLVE_VOLTAGE(l_hat, e, Pc_base, P, Bstack, V, K22_set, Peval, Beval, MODEL)
...
%     MODEL（可選）：'full'（預設）| 'diag'  —— 電壓模型的版本，見下。
...
%
%   [ADDED 2026-09-04 使用者拍板] MODEL 旗標 —— 兩個電壓模型版本擇一：
%     'full'  現行：激發 j 的電荷向量 G(:,j) 自由（36 個線性未知），H_V = G·V⁺ 滿矩陣。
%     'diag'  舊 Hall-sensor 模型（fix-ℓ 版，原 Hall_sensor_base_fix_dir/solve_d.m）：
%               b_ij = S_i · V_j · d ,   V_j = diag(V(:,j)) ,  d 為 6×1、**所有激發共用**
%             即「極 k 的電荷 = 自己 sensor 的讀值 × 自己的常數」：G_eff(k,j) = V(k,j)·d(k)。
%             d 對 b 線性，給定 (ℓ̂, e) 有閉式解（無疊代）：
%               d = ( Σ_j V_j (AᵀA) V_j )⁻¹ ( Σ_j V_j (Aᵀ b_j) )
%             電壓→電荷映射因此是對角陣 H_V = diag(d)。之後 gauge / NMAE / 𝒞κ 全部沿用：
%             ĝ_V·D̄ = H_V = diag(d) 恆成立，D̄ 會是對角陣、ĝ_V = (6/5)|d₁|；另回 rm.d 與 rm.MODEL。
%     ⚠ 兩版吃的 (ℓ̂, e) 是同一組（fitting 只看 WP 場、與 base / MODEL 無關）——
%       這正是舊流程「帶 current 校正好的 ℓ̂、ê 算 d」的意思。
%     ⚠ K22_set 只對 'full' 有意義（它改的是自由 G 的元素）；'diag' 給了會報錯。
    if nargin < 7,  K22_set = []; end
    if nargin < 8,  Peval   = []; end
    if nargin < 9,  Beval   = []; end
    if nargin < 10 || isempty(MODEL), MODEL = 'full'; end
    MODEL = validatestring(MODEL, {'full','diag'}, mfilename, 'MODEL');
    ...
    case 'diag'
        assert(isempty(K22_set), 'solve_voltage:K22diag', ...
               'K22_set 只適用 MODEL=''full''（它約束的是自由 G 的元素）');
        N_I = size(Bstack, 2);
        M   = A.' * A;                               % Σ_i S_iᵀS_i（6×6，與 j 無關）
        Ad  = zeros(6);   rhs = zeros(6,1);
        for j = 1:N_I
            Vj  = diag(V(:,j));                      % 該次模擬六顆 sensor 讀值擺對角
            Ad  = Ad  + Vj * M * Vj;
            rhs = rhs + Vj * (A.' * Bstack(:,j));
        end
        d   = Ad \ rhs;                              % 6×1 閉式解（無 gauge、無增益）
        G   = V .* d;                                % G_eff(k,j) = V(k,j)·d(k)，餵給下游 NMAE
        H_V = diag(d);                               % 電壓→電荷映射 = 對角
    end
    ...
    rm.MODEL = MODEL;                                 % [ADDED 2026-09-04] 'full' | 'diag'
    if strcmp(MODEL, 'diag'), rm.d = d; end           % 舊模型的 6×1 每極常數（= diag(H_V)）
end
