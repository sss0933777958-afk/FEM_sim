function ad = build_actuator_data(raw, cfg)
%BUILD_ACTUATOR_DATA  raw（measure frame, Tesla）→ ad（actuator frame, mT, canonical all-source）。
%   ad = BUILD_ACTUATOR_DATA(raw, cfg)
%   把純讀的 raw 場轉成校正可用的 actuator-frame 資料包（不只旋轉：另濾鐵、換單位、套 all-source 號誌）。
%   依 cfg.strategy 分支；B 換單位 ×1e3(T→mT) 並套 canonical all-source
%   （逐激發欄 cfg.s_source：只翻「raw 是 sink」的極 → Ba 直接 all-source，下游不再補翻）。
%   回 struct .Pa(N×3[m]) .r2(N×1) .Ba(N×3×N_I[mT]) .R_act(3×3) .Pc_base(3×6) .F(6×N_I) [.WP]。
...
%   需 filter_iron_nodes（hex）在 path。
    ...
    switch cfg.strategy
        ...
        case 'hex_magic'
            ...
            ad = struct('Pa',Pa, 'r2',r2, 'Ba',Ba, 'R_act',R_act, 'Pc_base',Pc_base, 'F',F);
        ...
        case 'ntu_flat'
            ...
            ad = struct('Pa',P_meas, 'r2',r2, 'Ba',Ba, 'R_act',eye(3), 'Pc_base',Pc_base, 'F',F, 'WP',WP);
