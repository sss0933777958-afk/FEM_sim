...
**用途**：Hall-sensor 每極 `d` 流程的數學/IO 輔助函式（一檔一函式）。**main.m 掛勾的 `build_sensor_geometry`、`extract_Vmat_interp` + 輸出階段 `emit_model_results` 已移到 `../main_function/`**（前兩支下方仍列說明）；本夾留其餘（build_S/extract_Vmat/…/solve_d）。
