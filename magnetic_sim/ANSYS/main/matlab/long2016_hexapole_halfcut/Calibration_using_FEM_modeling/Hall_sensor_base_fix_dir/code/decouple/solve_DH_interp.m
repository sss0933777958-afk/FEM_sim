...
SL = load(ellf,'ell','J'); ell_hat = SL.ell; J_fixdir = SL.J;
fprintf('載入 ℓ̂ = %.4f mm；fix_dir 自由電荷 cost J = %.6e\n', ell_hat*1e3, J_fixdir);
...
for j = 1:N_I
    ...
    Bj=-[dj.bx(airj),dj.by(airj),dj.bz(airj)]; B(:,:,j)=Bj(insel,:);
end
...
fprintf('\n=========== 內插版 滿 D_H 解耦（1000 點，ℓ̂=%.3f mm）===========\n', ell_hat*1e3);
fprintf('  D_H (6×6, 列=電荷極 P1..P6, 欄=sensor P1..P6)：\n');
for i=1:6, fprintf('   % .3e % .3e % .3e % .3e % .3e % .3e\n', D_H(i,:)); end
fprintf('  ‖diag‖=%.3e ‖off‖=%.3e off/diag=%.2f\n', diagn, offn, offn/diagn);
fprintf('  cost_J (內插滿 D_H)       = %.6e  [T^2]\n', cost_J);
fprintf('  cost_J (fix_dir 自由電荷) = %.6e  [T^2]（下界）\n', J_fixdir);
fprintf('==================================================================\n');
