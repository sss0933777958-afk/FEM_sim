...
ell0     = 0.5e-3;            % ell_hat 初值 [m]（= ell_design）
...
[P, Bstack, npts] = select_ball(D, R_select);                              % [沿用] Bstack=3Np×6 交錯堆疊
fprintf('選出 N_p = %d air 節點（R ≤ %g µm）；|B|max(球內) = %.4f T\n', ...
        npts, R_select*1e6, max(vecnorm(reshape(Bstack,3,[]),2,1)));

%% ---- 步驟2：擬合 ell_hat、e_hat（profile Dv；lsqnonlin）-------------------
[ell_hat, e_hat, J] = fit_bias(P, Bstack, D.Pc_base, ell0);                % [沿用]
Pc = make_Pc(e_hat, D.Pc_base);                                            % [沿用]
fprintf('擬合：ell_hat = %.4f mm | J = %.6e\n', ell_hat*1e3, J);
...
err = vecnorm(R3, 2, 1).';         % (npts*6)×1，單位 T

% 驗證：region err% 應 ≈ main_Dmatrix 印的數字
errpct = 100 * sqrt(sum(err.^2) / sum(Bstack(:).^2));
fprintf('驗證：numel(err) = %d（= npts*6 = %d）| region err = %.3f%% | median = %.4f mT | max = %.4f mT\n', ...
        numel(err), npts*6, errpct, median(err)*1e3, max(err)*1e3);
