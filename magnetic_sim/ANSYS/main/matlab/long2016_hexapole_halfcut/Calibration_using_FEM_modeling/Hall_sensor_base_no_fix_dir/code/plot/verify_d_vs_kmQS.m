%% verify_d_vs_kmQS.m  (18-param bias 版)
%  驗證:用 d 解出的 sensor 場  b = g_H·S·diag(d)·V_j
%        是否等於 hw.pdf eq(1) 的  b = (k_m/ℓ̂²)·S·Q̂_j ,  Q̂_j = (1/μ0)·diag(d)·V_j 。
%  關係:g_H = k_m/(ℓ̂²·μ0)  ⇒  兩式應逐元素相同(機器精度)。
%  bias 版:空間核用 18-param 電荷格點 Pc_18、工作點旋進 actuator 框(讀 calib_sensor_d_no_fix_dir.mat)。
clear; clc;
...
matf = 'G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\MATLAB_data\long2016_hexapole_halfcut\charge_fit\fitting_d\calib_sensor_d_no_fix_dir.mat';

cnst = mt_constants();
S = load(matf);                       % d(=d_final), gH, Vmat(all-source), ell_hat, Pc(=Pc_18), Rrot, R_select
d = S.d; Vmat = S.Vmat; gH = S.gH; ell = S.ell_hat; Pc = S.Pc; Rrot = S.Rrot;
...
fprintf('=== (2) 磁荷 Q̂ = (1/mu0) diag(d) Vmat ===\n');
for i=1:6, fprintf('   % .4e % .4e % .4e % .4e % .4e % .4e\n', Q(i,:)); end
fprintf('\n');

%% (3) 在 R<=R_select 真實節點上比兩種寫法(actuator 框、Pc_18 核)
...
P = (Rrot * Pall(insel,:).').'; Np = size(P,1);     % 旋進 actuator 框

km_l2 = cnst.k_m / ell^2;                             % k_m/ℓ̂²
maxabs = 0; sse = 0; ssb = 0;
for i = 1:Np
    Di = P(i,:).'/ell - Pc;                          % 3x6  (pbar - Pc_k), Pc = Pc_18(actuator)
...
fprintf('=== (3) 在 R<=%dum 全 %d 節點 × 6 sim 比較 ===\n', round(S.R_select*1e6), Np);
