...
% =========================================================================
% Current_base（電荷模型）：B = S·(^Bĝ_I·K̄·F)  → 電荷 Gc = ^Bĝ_I·K̄·F（no_fix_dir gauge_KI）。
% Voltage_base（電壓模型）：B = S·(^Bĝ_V·D̄·V) → 電荷 Gv = ^Bĝ_V·D̄·Vmat（Hall_sensor_base_no_fix_dir）。
% 兩者數學上恆等於 G=D^v=(AᵀA)⁻¹AᵀBstack（^Bĝ_I·K̄·F=G、^Bĝ_V·D̄=Ĥ_V 且 Ĥ_V·Vmat=G）→ 場/誤差/變異數相同
...
OUTDIR = ['G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\matlab\' ...
          'long2016_hexapole_halfcut\Calibration_using_FEM_modeling\Hall_sensor_base_no_fix_dir\figures'];   % <== 本副本輸出位置
...
[ell, e_hat] = fit_bias(P, Bstack, D.Pc_base, ell0);   % P/ell 均公尺（well-scaled）
Pc = make_Pc(e_hat, D.Pc_base);
A  = build_A(ell, Pc, P);
Dv = (A.'*A)\(A.'*Bstack);

% ---- Current_base：Gc = ^Bĝ_I·K̄·F ----
[K_bar, ghat_I_B] = gauge_KI(ell, Pc, P, Bstack, D.F);  % K̄、^Bĝ_I
Gc = ghat_I_B * K_bar * D.F;
err_c = vecnorm(reshape(A*Gc - Bstack, 3, npts*6), 2, 1).';

% ---- Voltage_base：Gv = ^Bĝ_V·D̄·Vmat ----
H = load(fullfile(CAL,'Hall_sensor_base_no_fix_dir','data',sprintf('calib_D_%s.mat',VARIANT)));  % Dmat(=Ĥ_V),D_bar,Vmat_p
ghat_V_B = (6/5)*H.Dmat(1,1);                           % ^Bĝ_V = (6/5)·Ĥ_V(1,1)（.mat field 'Dmat' 沿用）
Vmat = H.Vmat_p(:, apdl_to_paper_idx);                  % sensor paper × 激發 APDL
Gv = ghat_V_B * H.D_bar * Vmat;
