...
% EMIT_MODEL_RESULTS  Hall-sensor (18-param bias) 校正結果，Hall 表格式一檔輸出
%   D̄ / G / V / ℓ̂ / ^Bĝ_V / ê。純結果（無 caption）、單位標題（無因次/10^0 不標）。
%   獨立於 main.m（自足載 calib_D_*.mat 出 PDF）。
%   輸出：Hall_sensor_base_no_fix_dir/results/model_results_gap200um_mueq.pdf。

    VARIANT = 'gap_200um';
    here   = fileparts(mfilename('fullpath'));
    base   = fileparts(fileparts(here));                  % .../Hall_sensor_base_no_fix_dir
    out_dir = fullfile(base,'results');
    if ~exist(out_dir,'dir'); mkdir(out_dir); end

    S = load(fullfile(base,'data',sprintf('calib_D_%s.mat',VARIANT)), ...
             'D_bar','Dv_p','Vmat_p','ell_hat','g_V','E36');
    D_bar    = S.D_bar;      % 無因次（D̄(1,1)=5/6）
    G        = S.Dv_p;       % [mT]
    V        = S.Vmat_p;     % [mV]
    ell_hat  = S.ell_hat;    % [µm]
    ghat_V_B = S.g_V;        % [mT/mV]
    E36      = S.E36;        % 3×6 ê（actuator frame、無因次）
    ...
    emit_e(fid, 'e~[\mu\mathrm{m}]', S.ell_hat.*E36, pole, '');  % 3×6 物理偏移 e = ℓ̂·ê [µm]（per unit-reference 長度→µm）
