...
**資料來源 / 流向**：**載入** `MATLAB_data/.../charge_fit/calibration/calib_bias.mat`（no_fix_dir 校正結果：`ell_hat≈0.857 mm, Pc_18, R, F`）→ 重載 6-coil FEM 場（1 A）旋進 actuator 框 → `build_A` 建 `M, c` → page-2 建 sensor 幾何、抽 all-source `Vmat`、解 `d`、算殘差 → 存 `charge_fit/fitting_d/calib_sensor_d_no_fix_dir.mat`（**不蓋** fix-ℓ 版的 `calib_sensor_d.mat`）→ 輸出 4 個 `.tex` 到 `results/`。
