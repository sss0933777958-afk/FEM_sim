...
- `charge_field_err_hist_gap_200um.png` — fix-ℓ 模型 vs FEM（gap200）逐點逐激發向量差 |B_model−B_FEM|（6 激發合併，R=150 µm 球內 8774 點×6）。橫軸 mT、縱軸 count；median≈0.183 mT（參考 region err 3.19%，不顯示於圖）。由 `../code/plot/plot_charge_field_err_hist.m` 產生（同時存 `../data/field_err_hist_gap_200um.mat` 供疊圖用）。
