...
**資料來源 / 流向**：讀 `ANSYS_data/long2016_hexapole_halfcut/` 6-coil FEM 場（1 A）→ `select_ball` 取 R 內節點 → `fit_KI_fixl` → `region_field_err` → `write_KI_tex` 輸出 `results/fit_ball_R<R>um_<I>A.tex`；**並存解** `MATLAB_data/.../charge_fit/calibration/fit_fixl_R<RRR>um.mat`（`ell`/`gB`/`Khat`/`J`/`errpct`，供 `Hall_sensor_base_fix_dir` 載入 ℓ̂）。
