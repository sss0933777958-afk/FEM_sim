...
**資料來源 / 流向**：讀 `ANSYS_data/long2016_hexapole_halfcut/` 6-coil FEM 場（1 A）→ `select_ball` 取 R 內節點 → `fit_KI_fixl` → `region_field_err` → `write_KI_tex` 輸出 `results/fix_l/fit_ball_R<R>um_<I>A.tex`。
