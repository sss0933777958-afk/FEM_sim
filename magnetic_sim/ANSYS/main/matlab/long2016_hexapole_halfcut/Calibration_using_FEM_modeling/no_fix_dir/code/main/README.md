...
**內容**：
- `main.m` — 六極 18-param bias：頂部 config（MODE `single`/`sweep`、R 範圍 50:5:500 µm 或單一 150、I_actual=1 A、SHAPE=ball、ell0=0.5e-3、dataset='all'）；流程 load_coils_actuator → select_ball → fit_bias → make_Pc → gauge_KI → region_field_err → write_KbarI_tex。
