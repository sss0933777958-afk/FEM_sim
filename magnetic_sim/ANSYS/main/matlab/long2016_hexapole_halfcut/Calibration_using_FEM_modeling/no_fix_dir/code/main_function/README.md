...
**內容**：`load_coils_actuator`（載 6-coil FEM、旋 actuator）、`select_ball`、`fit_bias`（lsqnonlin fit {ℓ̂,ê17}）、`bias_resid`（殘差）、`make_Pc`（ê+Pc_base→電荷）、`build_A`（設計矩陣）、`gauge_KI`（profile g_j → ^Bĝ_I/K̄）、`region_field_err`、`calc_range_metrics`（σ_tot/iso_tot）、**`emit_model_results`**（輸出階段：讀 fit `.mat` 出結果 PDF 到 `../results/`）。
