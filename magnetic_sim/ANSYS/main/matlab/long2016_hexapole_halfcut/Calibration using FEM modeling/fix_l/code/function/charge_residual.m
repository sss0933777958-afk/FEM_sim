...
%     B(p) = gB * sum_i ( sum_j Khat_ij I_j ) * (p/ell - dhat_i)/||p/ell - dhat_i||^3
%   Inputs:
%     x        : packed [ell_mm; gB; Khat(free)]  (see unpack_params)
%     coil     : struct array, coil(k).p (Nx3 [m]), .bfem (3N x1 [T]), .pj (paper idx)
