...
**內容**：`plot_fixl_convergence.m`（fit 收斂）、`plot_P1_circuit_charge.m` / `plot_P2_circuit_charge.m`（單極磁路+電荷）、`plot_pole_circuit_charge_3d.m`（3D）、`make_R040_charge_figs.m`（R=40 µm 電荷圖批次）。
