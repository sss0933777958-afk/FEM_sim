%% gen_Vout_Vin_4p472.m
%  ------------------------------------------------------------------
%  Compute V_out/V_in 6x6 matrix using sensor placement at 4.472 mm
%  (variant of compute_B_bar_matrix.m which uses 4.572 mm).
%
...
%
%  Sensor placement (changed from 4.572 mm):
%      LOWER (P1/P3/P6): pole-local (4.472, 0.41, 0) mm relative to tip
%                        — 4.472 along milled flat, 0.41 perpendicular up.
%      UPPER (P2/P4/P5): pole-local (decomposed from 4.472 along cone slant
%                        + 0.41 outward normal):
%                          axial      = 4.472*cos(beta) - 0.41*sin(beta) = 4.3050
%                          transverse = 4.472*sin(beta) + 0.41*cos(beta) = 1.2795
%
%  FEM source: magnetic_sim/ANSYS/main/ANSYS_data/long2016_hexapole_halfcut/coilN
%      coil1 -> coil1_pre_fine_mesh (the matching half-cut run; the bare
%      'coil1' folder is an uncut-geometry run, do NOT use).
%
%  Outputs:
%    magnetic_sim/ANSYS/main/MATLAB_data/long2016_hexapole_halfcut/bs_matrix/Vout_Vin_4p472.mat
%    magnetic_sim/ANSYS/main/doc/Solve_B_matrix/long2016_hexapole_halfcut/scripts/Vout_Vin_4p472.tex
%  ------------------------------------------------------------------
...
N_disc_points = size(disc_local,1);             % 721
assert(N_disc_points == 721, 'disc point count mismatch');

%% --- Geometry: cone half-angle ---
beta_deg = atan2d(3.0, 15.0);                   % 11.31 deg
beta     = deg2rad(beta_deg);

%% --- Sensor placement: 4.472 mm (CHANGED from 4.572) ---
S_ALONG  = 4.472e-3;                            % NEW: was 4.572e-3
S_OFFSET = 0.41e-3;                             % unchanged

% LOWER pole: along milled flat (= along axis), perpendicular up
sensor_pl_lower = [S_ALONG; S_OFFSET; 0];
n_pl_lower      = [0; 1; 0];

% UPPER pole: along cone slant + outward normal, decomposed to pole-local
%   foot  = S_ALONG * (cos beta, sin beta, 0)
%   sensor = foot + S_OFFSET * (-sin beta, cos beta, 0)
...
%% --- Load FEM for 6 coils, sample 0.3 mm disc at each sensor ---
fprintf('\nSampling 721-pt disc at each sensor for 6 coils...\n');

B_matrix_at_Iin = zeros(6, 6);   % rows = sensor i (paper P), cols = excited coil paper-index
...
B_S        = (S_hall / I_in_A) * B_matrix_at_Iin;   % V/A
V_VV       = k_A * B_S;                              % V/V (dimensionless)
B_bar      = V_VV;                                   % alias: B_bar = V/V chain

fprintf('\n=== Diagonal comparison: V_out/V_in (mV/V scale) ===\n');
fprintf('  Pole   new (4.472)     old (4.572 from memory)   delta %%\n');
% Old diag from memory project_long_fei_B_bar: 1.10 1.58 1.06 1.66 1.57 1.20 (mV/V)
old_diag_mVV = [1.0978, 1.5763, 1.0600, 1.6551, 1.5723, 1.2005];
for i = 1:6
    new_mVV = V_VV(i,i);   % V/V (was *1e3 for old broken-unit mV/V display)
    delta   = (new_mVV - old_diag_mVV(i)) / old_diag_mVV(i) * 100;
...
mat_path = fullfile(data_dir, 'Vout_Vin_4p472.mat');
...
%% --- LaTeX (bmatrix, mV/V — clearly labeled) ---
%  Apply cosmetic sign convention: diagonal +|.|, off-diagonal -|.|
%  (matches existing Vout_Vin_kA0p36.tex output style)
...
tex_file = fullfile(tex_dir, 'Vout_Vin_4p472.tex');

fid = fopen(tex_file, 'w');
oc  = onCleanup(@() fclose(fid));

fprintf(fid, '%% Auto-generated by magnetic_sim/ANSYS/main/matlab/long2016_hexapole_halfcut/fit/gen_Vout_Vin_4p472.m\n');
fprintf(fid, '%% Sensor placement: 4.472 mm along surface, 0.41 mm normal\n');
