%% gen_Vout_Vin_4p572_gap200um_mueq.m
%  ------------------------------------------------------------------
%  Same as gen_Vout_Vin_4p572.m but reads from the gap200um_mueq variant
%  result dirs (coil{1..6}_gap200um_mueq/) — single-side 200um yoke-pole air
%  gap geometry. Both halfcut + gap200um_mueq effects baked in via FEM sim.
%
%  Outputs:
%    magnetic_sim/ANSYS/main/MATLAB_data/long2016_hexapole_halfcut/bs_matrix/Vout_Vin_4p572_gap200um_mueq.mat
%    magnetic_sim/ANSYS/main/doc/Solve_B_matrix/long2016_hexapole_halfcut/scripts/Vout_Vin_4p572_gap200um_mueq.tex
%  ------------------------------------------------------------------
...
fprintf('  Lower: axial=%.4f  transverse=%.4f\n', sensor_pl_lower(1)*1e3, sensor_pl_lower(2)*1e3);
fprintf('  Upper: axial=%.4f  transverse=%.4f\n', sensor_pl_upper(1)*1e3, sensor_pl_upper(2)*1e3);
...
%% --- Load FEM from gap200um_mueq variant dirs ---
fprintf('\nSampling 721-pt disc at each sensor for 6 gap200um_mueq coils...\n');

B_matrix_at_Iin_T = zeros(6, 6);   % TESLA

for k_apdl = 1:6
    coilname = sprintf('coil%d', k_apdl);
    coil_dir = fullfile(res_base, [coilname '_gap200um_mueq']);    % gap variant
    fprintf('  %s_gap200um_mueq: ', coilname);
...
B_matrix_at_Iin_T_raw = B_matrix_at_Iin_T;
...
%% --- UNIT CHAIN ---
S_hall = 130;                    % V/T
v_H    = S_hall * B_matrix_at_Iin_T;
B_matrix_mT = B_matrix_at_Iin_T * 1000;
I_in_A = 1.0;                    % smrt 5 standard: [V4 Long2016 verbatim] CURR_ARRAY=1.0
k_A    = 0.3614;
B_S    = v_H / I_in_A;
V_VV   = k_A * B_S;

fprintf('\n=== gap200um_mueq Diagonal (per-pole self-coupling) ===\n');
fprintf('  Pole    B_FEM (mT)   v_H (V)    B_S (V/A)   V/V\n');
for i = 1:6
    fprintf('  P%d      %+7.3f      %+7.4f    %+7.4f    %+7.4f\n', ...
            i, B_matrix_mT(i,i), v_H(i,i), B_S(i,i), V_VV(i,i));
end

%% --- Save .mat ---
data_dir = 'G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\MATLAB_data\long2016_hexapole_halfcut\bs_matrix';
if ~exist(data_dir, 'dir'); mkdir(data_dir); end
mat_path = fullfile(data_dir, 'Vout_Vin_4p572_gap200um_mueq.mat');
...
tex_file = fullfile(tex_dir, 'Vout_Vin_4p572_gap200um_mueq.tex');

fid = fopen(tex_file, 'w');
oc  = onCleanup(@() fclose(fid));

fprintf(fid, '%% Auto-generated by magnetic_sim/ANSYS/main/matlab/long2016_hexapole_halfcut/fit/gen_Vout_Vin_4p572_gap200um_mueq.m\n');
fprintf(fid, '%% Source: gap200um_mueq variant FEM (coil{1..6}_gap200um_mueq/)\n');
fprintf(fid, '%% Geometry: halfcut hexapole + 200um yoke-side air slab at each pole-yoke interface\n');
...
fprintf(fid, '\\noindent Geometry: Long Fei halfcut hexapole + \\textbf{single-side 200~$\\mu$m yoke-pole air gap}.\\\\\n');
fprintf(fid, 'Parameters: $k_A = %.4f$~A/V, $S_{hall} = %g$~V/T, $I_{in} = %.1f$~A.\\\\\n', k_A, S_hall, I_in_A);
fprintf(fid, 'Sensor placement: %.3f mm along pole surface + %.2f mm normal.\\\\\n', sensor_along_mm, sensor_offset_mm);
