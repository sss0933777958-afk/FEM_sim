%% gen_Vout_Vin_2p286.m
%  ------------------------------------------------------------------
%  Compute V_out/V_in 6x6 matrix using sensor placement at 2.286 mm
%  (sensor variant: half of original 4.572 mm; see also gen_Vout_Vin_4p472.m).
%
...
%
%  Sensor placement (changed: 4.572 -> 2.286 mm, halved):
%      LOWER (P1/P3/P6): pole-local (2.286, 0.41, 0) mm relative to tip
%      UPPER (P2/P4/P5): pole-local decomposition of
%                          2.286 along cone slant + 0.41 outward normal
%                          axial      = 2.286*cos(beta) - 0.41*sin(beta) = 2.1614
%                          transverse = 2.286*sin(beta) + 0.41*cos(beta) = 0.8506
%
...
%
%  Outputs:
%    magnetic_sim/ANSYS/main/matlab/long2016_hexapole_halfcut/bs_matrix/data/Vout_Vin_2p286.mat
%    magnetic_sim/ANSYS/main/doc/Solve_B_matrix/long2016_hexapole_halfcut/scripts/Vout_Vin_2p286.tex
%  ------------------------------------------------------------------
...
%% --- Hall sensor geometry (same as other variants) ---
sensor_diam_mm = 0.3;
...
%% --- Sensor placement: 2.286 mm (CHANGED: half of 4.572) ---
S_ALONG  = 2.286e-3;                            % CHANGED: 4.572 -> 2.286
...
fprintf('\n=== Diagonal |V_out/V_in| (mV/V): new (2.286) vs old (4.572) ===\n');
fprintf('  Pole   new (2.286)     old (4.572)         delta %% (abs)\n');
old_diag_mVV = [1.0978, 1.5763, 1.0600, 1.6551, 1.5723, 1.2005];   % from memory project_long_fei_B_bar
for i = 1:6
...
mat_path = fullfile(data_dir, 'Vout_Vin_2p286.mat');
...
tex_file = fullfile(tex_dir, 'Vout_Vin_2p286.tex');

fid = fopen(tex_file, 'w');
oc  = onCleanup(@() fclose(fid));

fprintf(fid, '%% Auto-generated by magnetic_sim/ANSYS/main/matlab/long2016_hexapole_halfcut/fit/gen_Vout_Vin_2p286.m\n');
fprintf(fid, '%% Sensor placement: 2.286 mm along surface, 0.41 mm normal\n');
