%% gen_Vout_Vin_3p572.m
%  ------------------------------------------------------------------
%  Compute V_out/V_in 6x6 matrix using sensor placement at 3.572 mm
%  (sensor variant: see also gen_Vout_Vin_{4p472,3p472,2p286}.m).
%
...
%
%  Sensor placement (changed: 4.572 -> 3.572 mm; baseline minus 1.0 mm):
%      LOWER (P1/P3/P6): pole-local (3.572, 0.41, 0) mm relative to tip
%      UPPER (P2/P4/P5): pole-local decomposition of
%                          3.572 along cone slant + 0.41 outward normal
%                          axial      = 3.572*cos(beta) - 0.41*sin(beta) = 3.4230
%                          transverse = 3.572*sin(beta) + 0.41*cos(beta) = 1.1030
%
...
%% --- Sensor placement: 3.572 mm (CHANGED) ---
S_ALONG  = 3.572e-3;                            % CHANGED: 4.572 -> 3.572 (baseline - 1.0)
S_OFFSET = 0.41e-3;
...
fprintf('\n=== Diagonal |V_out/V_in| (mV/V): new (3.572) vs old (4.572) ===\n');
fprintf('  Pole   new (3.572)     old (4.572)         delta %% (abs)\n');
...
tex_file = fullfile(tex_dir, 'Vout_Vin_3p572.tex');

fid = fopen(tex_file, 'w');
oc  = onCleanup(@() fclose(fid));

fprintf(fid, '%% Auto-generated by magnetic_sim/ANSYS/main/matlab/long2016_hexapole_halfcut/fit/gen_Vout_Vin_3p572.m\n');
fprintf(fid, '%% Sensor placement: 3.572 mm along surface, 0.41 mm normal\n');
