...
- `results/` — auto-gen `.tex`（`B_bar_matrix_0p6A`、`B_s_*`、`Vout_Vin_*`，含多 sensor 距離變體與 gap_200um 變體）。
...
- ⚠ sensor 資料夾陷阱（baseline vs gap_200um vs pre_fine_mesh 等）：讀結果前依 `result-read-safety` 規則核指紋。
