function plot_P1_circuit_with_flux_steelonly()
% PLOT_P1_CIRCUIT_WITH_FLUX_STEELONLY
% Same layout as plot_P1_circuit_with_flux.m but with FEM nodes filtered to
% STEEL only before building scatteredInterpolant. Each Q point's tet uses
% only steel nodes (no air contamination).
%
% Steel mask covers: P1 cone+cylinder, yoke block (right side), post column.
% Other 5 poles excluded (they're at y!=0, don't affect P1's y=0 slice much).
    ...
    fprintf('Building scatteredInterpolant (steel-only, for Phi integration)...\n');
    ...
    % B field quiver (steel-only interpolant)
    grid_x = 130; grid_z = 38;
    xc = linspace(-3, 60, grid_x);  zc = linspace(-18, 2, grid_z);
    [Xg, Zg] = meshgrid(xc, zc); Xg = Xg(:); Zg = Zg(:);
    % Side view uses FULL interpolant (visualization of true B field everywhere)
    ...
    % Truncate curve to peak (drop the falling tail after Phi_max)
    [phi_pk, k_pk] = max(Phi_uWb);
    x_plot   = x_query_mm(1:k_pk);
    Phi_plot = Phi_uWb(1:k_pk);

    plot(x_plot, Phi_plot, 'b-', 'LineWidth', 2.2);

    ymax = phi_pk * 1.10; ymin = -ymax*0.05;
    ...
    grid on; box on;
    xlim([-2, x_query_mm(k_pk)]); ylim([ymin ymax]);
    ...
    title('P1 axial flux profile  (steel-only interp,  half-disc with R(x))', ...
          'FontSize', 12, 'Interpreter','tex');

    %% --- Save ---
    out_path = fullfile(out_dir, 'P1_circuit_with_flux_steelonly.png');
    exportgraphics(fig, out_path, 'Resolution', 250);
    fprintf('Saved: %s\n', out_path);
    fprintf('  Phi_max = %.4f µWb @ x = %.2f mm\n', phi_pk, x_query_mm(k_pk));
end
...
% Steel mask for P1's side-view region (y=0 slice).
% Includes: P1 cone (D-shape after halfcut), P1 cylinder, yoke block, P1 post.
% Inputs in metres (x_m, y_m, z_m); geometry params in mm (will convert).
    x  = x_m * 1e3;
    y  = y_m * 1e3;
    z  = z_m * 1e3;

    % Radial distance from P1 cone/cylinder axis (axis at y=0, z=Z_CONE)
    ...
    % --- Yoke block (rectangular, lower side: x ∈ [x_end, X0_LOW+11], z ∈ [-PROT_H-10, -PROT_H]) ---
    in_yoke_lower = (x >= x_end_mm) & (x <= X0_LOW_mm + 11) & ...
                    (z >= -(PROT_H_mm + 10)) & (z <= -PROT_H_mm) & ...
                    (abs(y) <= 11);  % yoke half-width approx

    % Yoke step (x ∈ [x_end, POST_X1], z ∈ [-PROT_H-2, -PROT_H])
    POST_X1_mm = X0_LOW_mm - PROT_R_mm;
    in_yoke_step = (x >= x_end_mm) & (x <= POST_X1_mm) & ...
                   (z >= -(PROT_H_mm + 2)) & (z <= -PROT_H_mm) & ...
                   (abs(y) <= 11);

    % --- P1 post (vertical cylinder, R=PROT_R, z ∈ [-PROT_H, 0]) ---
