...
- `data/` — 本組繪圖會讀的 `.mat`（規則 #2，2026-06-26 遷入）：`flux_profile`（`P1/P2_flux_profile*`）、`joint_6coil_40um_fit`、`bh_saturation_sweep`。注意：本組另需 `Bbar_S_4p572.mat`（col_sign）讀自 `../bs_matrix/data/`（跨夾，產者 bs_matrix）。

**資料來源 / 流向**：讀 ANSYS_data 的 `coilN/coilN_{coord,bfield}_*.dat`（透過 `ansys_path` / `import_ansys_data` + `filter_iron_nodes`）+ 部分圖讀本組 `data/` 與 `../bs_matrix/data/` 的 `.mat` → 轉 WP frame、套 source 符號慣例 → 直接畫真實 FEM 節點 → 輸出 PNG 到本組 `figures/`。FEM 在 1A 解，繪圖不縮放。
