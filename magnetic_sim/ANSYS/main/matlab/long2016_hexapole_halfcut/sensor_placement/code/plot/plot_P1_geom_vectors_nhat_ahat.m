function plot_P1_geom_vectors_nhat_ahat()
% PLOT_P1_GEOM_VECTORS_NHAT_AHAT
%
...
%
% Sensor-end value: |B·n̂| at d = 2 mm marked in Tesla.
%
...
%       I_in = 1.0 A, halfcut geometry.
    ...
    text(anchor(1) - 0.6, anchor(2) - 0.45, 'axis (r=0)', ...
    ...
    for j = 1:N
        ...
        Bx = F_bx(pos_x*1e-3, 0, pos_z*1e-3);
        Bz = F_bz(pos_x*1e-3, 0, pos_z*1e-3);
    ...
    %% --- Draw arrows + dashed â direction guides ---
    h_B = []; h_n = []; h_a = []; h_d = [];
    for j = 1:N
        x0 = pos_all(j,1);  z0 = pos_all(j,2);

        % Dashed gray guide line from sample → P1 tip (â direction)
        h_dash = plot([x0, tip_x_p1], [z0, tip_z_p1], ...
        ...
        if j == 1
            h_B = h_full;  h_n = h_alpha;  h_a = h_beta;  h_d = h_dash;
        end
    end
    ...
    legend([h_B, h_n, h_a, h_d], ...
        {sprintf('$\\mathbf{B}$ (FEM, scale: 1\\,mm = %.3f\\,T)', 1/scale_B), ...
         ...
         '$\hat{\mathbf{a}}$ direction (toward P1 tip)'}, ...
    ...
    title('P1 baseline (Long2016 verbatim) -- flux vectors and $\hat{\mathbf{n}}$ component', ...
          'FontSize', 13, 'Interpreter', 'latex');

    %% --- Save ---
    out_path = fullfile(out_dir, 'P1_geom_vectors_nhat_ahat.png');
