% plot_lower_pole_sensor_placement_1p572.m
% ----------------------------------------------------------------------
% Variant: sensor at 1.572 mm along axis (3.0 mm closer to tip than baseline 4.572).
% All other geometry/style identical to plot_lower_pole_sensor_placement.m.
% Output: lower_pole_sensor_placement_1p572.png
% ----------------------------------------------------------------------
...
plot(ax, [s_along s_along], dim_y_top + [-0.10 0.10], '-', ...
     'Color', [0.80 0.10 0.10], 'LineWidth', 1.4);
text(ax, s_along/2, dim_y_top + 0.15, '1.572 mm', ...
...
ylabel(ax, 'y [mm]');
title(ax, 'Lower Pole Sensor Placement — Side View (milled half-cone, mm) — sensor 1.572 mm');
...
out_path = fullfile(out_dir, 'lower_pole_sensor_placement_1p572.png');
