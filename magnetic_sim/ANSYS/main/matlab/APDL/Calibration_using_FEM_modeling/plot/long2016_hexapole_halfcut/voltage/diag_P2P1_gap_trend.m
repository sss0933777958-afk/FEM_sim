...
%   注意：sensor B·n+ 為「局部單點內插」估計（baseline 粗網格 sensor 圓柱內節點少）。
    ...
    figdir = ['G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\matlab\' ...
              'long2016_hexapole_halfcut\Calibration_using_FEM_modeling\voltage_base\figures'];
    ...
    for k=1:numel(gaps)
        ds=import_ansys_data(fullfile(rr,sprintf('gap%dum_mueq',gaps(k)),'coil1'),'all','coil1');
