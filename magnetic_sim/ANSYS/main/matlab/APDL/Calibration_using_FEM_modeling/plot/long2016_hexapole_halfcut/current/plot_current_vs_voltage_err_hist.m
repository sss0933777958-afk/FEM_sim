...
% =========================================================================
% Current_base（電荷模型）：B = S·(^Bĝ_I·K̄·F)  → 電荷 Gc = ^Bĝ_I·K̄·F（current_base gauge_KI）。
% Voltage_base（電壓模型）：B = S·(^Bĝ_V·D̄·V) → 電荷 Gv = ^Bĝ_V·D̄·Vmat（voltage_base）。
...
OUTDIR = ['G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\matlab\' ...
          'long2016_hexapole_halfcut\Calibration_using_FEM_modeling\current_base\figures'];   % <== 本副本輸出位置
...
[K_bar, ghat_I_B] = solve_KI_bar_gain(ell, Pc, P, Bstack, D.F);  % K̄、^Bĝ_I
Gc = ghat_I_B * K_bar * D.F;
err_c = vecnorm(reshape(A*Gc - Bstack, 3, npts*6), 2, 1).';

% ---- Voltage_base：Gv = ^Bĝ_V·D̄·Vmat ----
H = load(fullfile(CAL,'voltage_base','data',sprintf('calib_D_%s.mat',VARIANT)));  % Dmat(=Ĥ_V),D_bar,Vmat_p
