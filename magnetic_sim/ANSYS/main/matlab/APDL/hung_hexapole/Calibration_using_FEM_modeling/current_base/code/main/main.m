...
VARIANT     = 'no_gap';         % FEM 變體子夾（gap_calibrate = 逐極單盤氣隙校準）

%% ---- paths ---------------------------------------------------------------
TREE = ['G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\matlab\' ...
        'hung_hexapole\Calibration_using_FEM_modeling\current_base'];
...
%% ---- 常數 + 慣例 ---------------------------------------------------------
cnst = mt_constants();
% 工作空間變體（R700/R300…）的 graded mesh 把 WP 置於原點（baseline SPH_OFST=-12.71mm 不適用）。
if ~isempty(regexp(VARIANT,'^R\d+$','once')), cnst.SPH_OFST = 0; end
% [MODIFIED 2026-07-17] hung = identity（coil k 激發 paper P k）。**不可抄 long2016 的 [1,3,6,5,2,4]**：
%   那是 long2016 deck 的建構順序（下極 0/120/240 先、上極 60/180/300 後），不是物理。
%   hung 的 deck 照 paper 序建（mesh/_mesh_R700.txt 的 fc_ra(k) 直接是 P k 的方位：
%   fc_ra=[0,180,120,300,60,240]、coil 迴圈 REAL,fc_pi）。實測佐證：raw B̂ 在 WP 對 P k tip
%   方向內積全 +0.999、G 對角占優。map 是 per-model，見 rule pole-coil-numbering.md。
apdl_to_paper_idx = [1, 2, 3, 4, 5, 6];                 % coil k excites this paper pole (hung: identity)
