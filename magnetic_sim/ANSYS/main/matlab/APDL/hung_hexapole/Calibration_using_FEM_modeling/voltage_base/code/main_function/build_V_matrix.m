...
% BUILD_V_MATRIX  sensor 電壓矩陣 V̄（standard 粗網格「真·FEM tet 重心內插」版）。
%   (Was extract_Vmat_interp; renamed 2026-07-16 — builds the Hall-sensor voltage matrix V̄.)
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
    ...
    if nargin < 12|| isempty(coils),     coils     = 1:6;        end  % [ADDED 2026-07-13] 要抽的 coil 清單（預設全 6；單 coil 實驗如 gap400/gapdist coil1 傳 coils=1）
    ...
    for kc = coils
        ...
        ds = import_ansys_data(fullfile(results_root, variant, cn),'all',cn);   % [MODIFIED] standard→variant（同網格、只換場來源）
