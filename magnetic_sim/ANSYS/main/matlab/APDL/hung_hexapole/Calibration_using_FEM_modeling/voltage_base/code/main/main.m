%% main.m -- Hall-sensor 統一校正 driver（voltage_base）：輸出 V̄, D̄, ^Bĝ_V
% =========================================================================
%  🔴🔴 [WARNING 2026-07-17] **這整包 voltage_base 是從 long2016 複製來的空殼，尚未適配 hung，別直接跑。**
%    跑之前必須先修下面兩件事（否則結果會「靜默錯」——數值不爆、只是錯）：
%
%    (1) **編號**：hung 的 deck 是 identity（coil k = paper P k），**不是** long2016 的 [1,3,6,5,2,4]。
%        本檔 :51 以及 code/plot/ 底下 10 支腳本仍硬寫 [1,3,6,5,2,4] = 錯的（current_base/main.m 已修為 identity）。
%
%    (2) **號誌**：hung 的 raw FEM 是**六極全 sink**（deck 用 +TURNS；實測 raw B̂ 對每顆 P k tip 內積全 +0.999），
%        全域 negate(-B_FEM) 就已一次到位讓六顆全 source。因此本包沿用 long2016 的
%        `ismember(apdl_to_paper_idx(j),[1 3 6]) → exc_sign(j)=-1`「翻下極」邏輯
%        （本檔 :59、function/extract_Vmat.m:78、function/extract_Vmat_interp_center.m:68）
%        **對 hung 不適用**——那是為 long2016「下極 sink / 上極 source」混合 raw 設計的。
%        ⚠ 只把 map 改成 identity、卻留著這個 exc_sign = 換一種錯，更難察覺 → 要改就兩件一起重新設計。
%
%    (3) **hung 目前根本沒有 sensor 資料**（ANSYS_data/hung_hexapole/data/R700/coil1/ 只有
%        *_{coord,bfield}_{all,wp}.dat，無 sensor .dat）。data/ 與 results/ 內現存的
%        calib_*_gap200um_mueq.* 全是複製 long2016 時留下的產物（時間戳 = 資料夾建立時間），不是 hung 的結果。
%
...
%% ---- ① 參數 --------------------------------------------------------------
USE_BIAS   = false;            % false = single-parameter（在軸）; true = 18-param bias（離軸）
VARIANT    = 'no_gap';         % FEM 變體：no_gap / gap_calibrate / gap_200um
N_I        = 6;                % FEM 模擬次數 = 6 個單線圈解
R_select   = 150e-6;           % 取點半徑 [m]
I_actual   = 1;                % 驅動電流 [A] = FEM 激發（1 A）
TURNS      = 50;               % 線圈匝數（記錄用；per-Ampere 模型不引用）
S_hall     = 130;              % Hall 靈敏度 [mV/mT]（EQ-730L）
ell0       = 0.5e-3;           % ℓ̄ 初值 [m]（fitting 在 SI）
n_uniform  = 10000;            % sensor 圓柱內均勻取樣點數（內插平均）
...
[D_bar, ghat_V_B, H_V] = solve_D_bar_gain(Dv, Vmat);   % Ĥ_V 內部算、不印
...
gen_B_matrix(VARIANT, USE_BIAS);          % 另出一份 B 矩陣 PDF（V_out/V_in）
