...
%   variant       (選填) FEM 變體子夾，預設 'gap_200um'。
% 輸出（暫時，隨步驟成長）：
...
% -------------------------------------------------------------------------
    if nargin < 2 || isempty(results_root)
        results_root = ['G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\' ...
                        'ANSYS_data\hung_hexapole\data'];
    end
    if nargin < 3 || isempty(variant), variant = 'gap_200um'; end
    ...
    gdir = fullfile(results_root, variant, 'coil1');
