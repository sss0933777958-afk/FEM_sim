...
%   S_hall              霍爾靈敏度 [mV/mT]（EQ-730L = 130；數值同 V/T，配合 B[mT] 得 V[mV]）
...
% 輸出：
%   Vmat      6×6 sensor 電壓 [mV]（列=sensor 極 P1..P6，欄=激發 coil1..6，已翻號）
...
% -------------------------------------------------------------------------
    ...
    for kc = 1:6                                               % 逐線圈 kc（= 第 j=kc 次模擬）
        ...
        da = import_ansys_data(fullfile(results_root, variant, cn),'all',cn);  % 載 'all' 全域真實節點（variant 子夾）
