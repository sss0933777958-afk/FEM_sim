function plot_P2_circuit_with_flux_steelonly()
% PLOT_P2_CIRCUIT_WITH_FLUX_STEELONLY
% Same layout as plot_P2_circuit_with_flux.m, with FEM nodes filtered to
% STEEL only before building scatteredInterpolant for Phi(s) integration.
% Side view uses full interpolant (visualization correctness).
%
% P2 = upper pole, azimuth 180°, tilted +36.59°, full cone (no halfcut).
    ...
    res_dir = 'G:\my_workspace\code\FEM_sim\magnetic_sim\ANSYS\main\ANSYS_data\long2016_hexapole_halfcut\data\standard\coil5';
    ...
    POST_R_mm    = PROT_H/2;        % half-width of upper post

    %% Cylinder length (along pole axis from cone_end to end_upper)
    ...
    %% --- Filter to STEEL only (P2 cone + cylinder + yoke + post + block) ---
    ...
    % B field quiver (full interpolant, for correct visualization)
    ...
    title('P2 magnetic flux side view  (steel-only interpolation, upper pole tilted +36.59°)', 'FontSize', 13);

    %% --- Bottom: axial flux profile (steel-only interp) ---
    ...
    s_plot   = s_query_mm(1:k_pk);
    Phi_plot = Phi_uWb(1:k_pk);

    plot(s_plot, Phi_plot, 'b-', 'LineWidth', 2.2);
    ...
    grid on; box on;
    xlim([-2, s_query_mm(k_pk)]); ylim([ymin ymax]);
    ...
    title('P2 axial flux profile  (steel-only interp, full disc with R(s) taper)', ...
          'FontSize', 12, 'Interpreter','tex');

    %% --- Save ---
    out_path = fullfile(out_dir, 'P2_circuit_with_flux_steelonly.png');
    exportgraphics(fig, out_path, 'Resolution', 250);
    fprintf('Saved: %s\n', out_path);
    fprintf('  Phi_max = %.4f µWb @ s = %.2f mm\n', phi_pk, s_query_mm(k_pk));
end
...
% Steel mask for P2 (upper pole, tilted).
% All inputs in metres.

    % Project nodes onto pole axis from tip
    ...
    % Yoke (rectangular slab, side view bbox + half-width 11 mm in y)
    in_yoke = (x_m >= yoke_x_m(1)) & (x_m <= yoke_x_m(2)) & ...
              (z_m >= yoke_z_m(1)) & (z_m <= yoke_z_m(2)) & ...
              (abs(y_m) <= 11e-3);

    % Block above post (Long Fei block)
