...
- `data/` — 本組繪圖會讀的 `.mat`（規則 #2）：`flux_profile`（`P1/P2_flux_profile*`）、`joint_6coil_40um_fit`、`bh_saturation_sweep`、**`Bbar_S_4p572.mat`（col_sign；2026-07-10 由已撤的 `bs_matrix/data/` 移入本組）**。

**資料來源 / 流向**：讀 ANSYS_data 的 `coilN/coilN_{coord,bfield}_*.dat`（透過 `ansys_path` / `import_ansys_data` + `filter_iron_nodes`）+ 部分圖讀本組 `data/` 的 `.mat`（含 `Bbar_S_4p572.mat`）→ 轉 WP frame、套 source 符號慣例 → 直接畫真實 FEM 節點 → 輸出 PNG 到本組 `figures/`。FEM 在 1A 解，繪圖不縮放。
