...
- `plot_bias_field_err_hist.m`（18-param bias 模型 vs FEM 場向量差誤差直方圖，**VARIANT='no_gap'**，選項①；沿用 load_coils_actuator/select_ball/fit_bias/make_Pc/build_A。場模型 = main_Dmatrix 的 A·g_j，故誤差與其一致；存 err **+ P1 電荷 `charge_act_P1`（actuator）** 到 `../../data/field_err_hist_no_gap.mat`）。
...
- `plot_charge_circuit_sideview.m`（**fix + no_fix 兩張** coil1(P1) no_gap 磁路箭頭側視圖：**畫布 measure/APDL frame**（x:−2~2mm、z:−14~−12mm、y=0 薄片、WP 在 z≈−12.711 非原點、**不標 WP**）、每格最近 y=0 真實節點 turbo quiver；**P1 charge 粉點**由各套件 `field_err_hist_no_gap.mat` 的 `charge_act_P1`（actuator）經 `R_act.'`+SPH_OFST 轉回 measure；一支腳本迴圈 PKG={fix,no_fix} → `fix_dir/figures/` + `no_fix_dir/figures/charge_circuit_sideview.png`。軸強制 `Exponent=0` 免 ×10⁴ 撞標題）。
