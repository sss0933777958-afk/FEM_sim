...
%
%  REUSES（不重寫；同 current_base/main.m）：load_coils(+variant), select_ball, fit_KI_fixl, charge_residual。
...
[ell, ghat_I_B, K_bar, J] = fit_KI_fixl(coil, Pc_base, I_actual); % [FIXED] actuator 框：電荷方向 Pc_base（同 main.m）；傳 dhat(measure) 會與 actuator coil.p 不一致而發散
...
charge_act_P1 = ell * Pc_base(:,1);                               % [ADDED] P1 電荷 (actuator frame, m) = ell*Pc_base(:,1)
...
save(fullfile(datadir, sprintf('field_err_hist_%s.mat', VARIANT)), ...
     'err','errpct','VARIANT','model_label','ell','charge_act_P1');
