...
%   跟 current_base 同風格（P1 下極半錐 / P2 上極全錐 + 粉色磁荷點 + charge→WP 虛線標 ℓ̂ + WP，
...
%   gap200um_mueq、R=150µm；輸出 current_base/figures/charge_positions_P1P2_3d.png。
    ...
    %% ---- current_base 的 ℓ̂（canonical gap200um_mueq）→ fix 磁荷（WP 框, mm）----
    Sf = load(fullfile(calroot,'current_base','data','fit_fixl_R150um_gap200um_mueq.mat'),'ell');
    ...
    [ell_nofix, e_hat, Pc] = fitting(P, Bstack, D.Pc_base, 0.5e-3, true);   % 18-param bias；回 (ℓ̄,ê,Pc)；ell_nofix [m]
