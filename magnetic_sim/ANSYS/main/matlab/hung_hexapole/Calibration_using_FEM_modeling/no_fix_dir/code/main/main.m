...
VARIANT     = 'gap_200um'; % [MODIFIED] FEM 變體子夾（'standard' = baseline；'gap_200um' = gap200 2 段式 μ_eff）
...
for R_um = R_um_list
    ...
    save(fullfile(cal_dir, sprintf('fit_bias_R%03dum%s.mat', R_um, vtag)), ...
         'ell','gB','Khat','e_hat','J','R_um','I_actual','SHAPE','VARIANT', ...
         'C_mean','kappa_mean','C_min','kappa_worst','Np_range');            % [MODIFIED] + C_mean/kappa_mean
    fprintf('R=%3d um | npts=%6d | ell=%.2f µm | ^Bg_I=%.4e mT/A | C_mean=%.4g (mT/A)^3 | kappa_mean=%.4f (Np=%d, C_min=%.4g, kappa_worst=%.4f)\n', ...
            R_um, npts, ell, ghat_I_B, C_mean, kappa_mean, Np_range, C_min, kappa_worst);
end
