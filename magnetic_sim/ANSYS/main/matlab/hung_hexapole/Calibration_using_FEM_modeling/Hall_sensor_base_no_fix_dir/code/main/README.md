...
（`VARIANT='gap_200um'`、`R_select=150 µm`、`I=1 A`、`S_hall=130 V/T`、`n_uniform=10000`）。
...
**流程（五步）**：
1. **載 6-coil FEM**（`load_coils_actuator`，`gap_200um`）→ `select_ball` R≤150 µm 球 → all-source（literal flip-sink：只翻下極 P1/P3/P6）。
...
- `.mat` `../../data/calib_D_gap_200um.mat`（**.mat field 名沿用** `Dmat/D_bar/g_V/Dv_p/Vmat_p/ell_hat/e_hat/E36/...`；ê 保留供下游）。

**預期數值（gap_200um, R=150 µm；原生單位）**：ℓ̂≈**856.66 µm**（18-param bias）、region err≈0.46%、recon~1e-16、^Bĝ_V≈**7.12e-3 mT/mV**、V 對角≈**1000 mV**、G 對角全正。
