...
%
%  DATA   : variant 'gap_200um'（GRADED mesh + support-base μ_eff），coil1..6（all-source）。
...
%  Output : .mat -> 本包 data/（field 名不變，含 ê）；PDF（V/D̄/^Bĝ_V/Ĥ_V）由 code/main_function/emit_model_results.m 產生。
...
%% ---- config ----------------------------------------------------------------
VARIANT    = 'gap_200um';  % FEM 變體：coil1..6 場來源（gap200 2 段式 μ_eff）
...
AXIAL_TOL  = 0.10e-3;          % [ADDED] sensor 圓柱厚度 [m]：canonical 0.10e-3；設 1e-6 = 1µm 薄片 what-if（檔名加 _h1um、不蓋 canonical；sensor_r 維持 Ø0.3mm）
...
addpath(fullfile(CAL,'no_fix_dir','code','main_function'));          % 沿用：load_coils_actuator/select_ball/fit_bias/make_Pc/build_A
...
[U_hv, S_hv, W_hv] = svd(H_V);                 % Ĥ_V 奇異值分解 Ĥ_V=U·Σ·Wᵀ（Σ=S_hv，單位 mT/mV）
kappa = S_hv(1,1) / S_hv(6,6);                 % 條件數 σ1/σ6（量 Ĥ_V 縮放/病態；fix vs no_fix 可比）
