...
**內容**：`code/main/main.m`（主程式，config 在頂部）、`code/main_function/`（載場/擬合/select_ball）、`code/function/`（模型數學輔助函式）、`code/plot/`（`plot_gain_iso_index.m` 直方圖、`plot_ref_planes_3d.m` 參考面、`plot_svd_heatmaps_2d.m` 離散熱圖）、`results/`（auto-gen `.tex`）、`data/`（`.mat` 成果，規則#2）、`figures/`（圖）。
