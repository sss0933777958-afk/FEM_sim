...
**用途**：fix-ℓ 校正的唯一驅動程式。
**內容**：`main.m` — 頂部 config（MODE `single`/`sweep`、R 範圍 50:5:500 µm 或單一 150、I_actual=1 A、SHAPE=ball）；流程 load_coils → select_ball → fit_KI_fixl → region_field_err → write_KI_tex。
