...
% =========================================================================
    ...
    %% ---- 兩張圖 -----------------------------------------------------------
    % g_I 的 y 刻度使用者指定為整數，不走自動取刻度。
    % [MODIFIED 2026-08-17] 刻度由 single [9 10] / eighteen [10 11] 各加一格 →
    %   [8 9 10] / [10 11 12]（仍是整數、奇數個、等距）。原因：safety_r 100um->0 的修正
    %   讓 R>=440 的取樣球不再被挖洞，R=500 的 conv 值移到 single 8.582、eighteen 11.162，
    %   都落在原本的 [t(1)-0.15, t(end)+0.15] 之外 → 觸發下方 assert。
    mk('single',   S.R_um, S.ell_f1, S.ell_c1, S.gI_f1, S.gI_c1, ...
       [0.05 0.10 0.95], S.N1, r_e1, r_g1, figdir, [8 9 10]);
    mk('eighteen', S.R_um, S.ell_f2, S.ell_c2, S.gI_f2, S.gI_c2, ...
       [0.85 0.10 0.10], S.N2, r_e2, r_g2, figdir, [10 11 12]);
end
