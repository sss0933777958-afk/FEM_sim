...
% =========================================================================
    ...
    %   （使用者指定：不要小數點）；資料最大 10.93 → 最上刻度 12 把曲線夾住。
    % [MODIFIED 2026-08-14] **所有點都畫**（使用者指定）—— 含 single 在 R=20/40 的
    %   g_I = -40.13 / -14.85 mT/A（小球內六顆電荷近簡併、最小平方解翻負）。
    %   ⚠ 連帶：縱軸必須下探到 -45，其餘 23 個點（9~11 mT/A）會被壓在頂端。
    %   刻度取整數、等距 15，且**把曲線完全夾住**（資料 -40.13 ~ 10.93 → 最低刻度 -45、
    %   最高刻度 15 都在資料之外）；上下各留 3 的裕度，不留過多空白。
    YL2 = [-48 18];   YT2 = -45:15:15;
