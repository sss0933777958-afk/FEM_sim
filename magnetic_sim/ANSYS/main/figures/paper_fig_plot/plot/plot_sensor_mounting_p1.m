function plot_sensor_mounting_p1(SOFF, FACE, WITHFIELD, SRC, MODEL)
...
%   輸出 → figures/paper_fig/Section3_A/sensor_mounting_tip40_P1.png
    ...
    fdB = rot(axc,-beta);
    if strcmpi(FACE,'upper_cone')   % [ADDED] 填滿極 = **完整錐**（上下錐面對稱），非半切平頂
        fdA = rot(axc, beta);
        tha = linspace(angf-hw, angf+hw, 80);  arc = C + rf*[cos(tha); sin(tha)];
        poly = [arc(:,1) + Lsl*fdA, arc, arc(:,end) + Lsl*fdB];
    else
    ...
    if strcmpi(FACE,'flat') || strcmpi(FACE,'upper_cone')   % 箭頭朝上 → 下方靠磁極下錐緣撐開
    ...
    if WITHFIELD
        ...
        if strcmpi(SRC,'maxwell')
            ...
            cs = fullfile(here, 'data', 'mount_field_P1_maxwell_src.mat');
            if exist(cs,'file')
        ...
        cf = fullfile(here, 'data', sprintf('mount_field_P1_%s_%s.mat', lower(FACE), lower(SRC)));
    ...
    BASEN = 'sensor_mounting_tip40_P1';
    if ISZHI, BASEN = 'sensor_mounting_zhi_P1'; end
    if strcmpi(FACE,'upper_cone'), BASEN = 'sensor_mounting_filled';  end        % [ADDED] 填滿極另命名
