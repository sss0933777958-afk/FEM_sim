...
% =========================================================================
    ...
    SIDE_VIEW = true;                                    % [ADDED 2026-08-14] true=側視(0,0)、false=原本的 3D (-45,-20)
    ...
    %   資料、取樣、幾何一律不動，只換視角 + 框線。
    %   側視時 y 軸邊看（深度塌陷）→ 手動 draw_box_edges3 會把前後框邊疊成同一條，
    %   改用 box on（此時投影就是一個乾淨矩形，等同風格①粗體框圖）、並拿掉 y 刻度。
    if SIDE_VIEW
        view(ax,0,0);   box(ax,'on');
    else
        view(ax,-45,-20);   box(ax,'off');
    end
    ...
    set(ax,'XTick',[2 4 6 8],'ZTick',[-2 -1 0]);                   % 內縮 tick(L=8 重新框)
