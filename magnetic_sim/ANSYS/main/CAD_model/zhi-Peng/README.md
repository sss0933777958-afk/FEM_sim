...
- `STEP/` — 目前為**空夾**（尚未匯出 STEP；需要時依 `reference/workflows/cad-export.md` 產出）。

**資料來源 / 流向**：pipeline 起點。SolidWorks → STEP → `IGES/zhang_quadrupole/` → `model_check/zhang_quadrupole/` → `apdl/zhang_quadrupole/geom` → ANSYS。目前下游 IGES 只有 `Zhang_Quadrupole_Lp0p405.iges`。
