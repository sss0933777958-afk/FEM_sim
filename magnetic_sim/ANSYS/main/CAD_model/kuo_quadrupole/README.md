...
**資料來源 / 流向**：此處 = pipeline 起點。SolidWorks 設計 → STEP 匯出 → 下游 `IGES/kuo_quadrupole/` → `model_check/kuo_quadrupole/` → `apdl/kuo_quadrupole/geom` → ANSYS solve → `ANSYS_data/` → `matlab/`。出檔 SOP 見 `doc/workflows/cad-export.md`；STEP 抽參數見 `doc/workflows/step-to-apdl.md`。
