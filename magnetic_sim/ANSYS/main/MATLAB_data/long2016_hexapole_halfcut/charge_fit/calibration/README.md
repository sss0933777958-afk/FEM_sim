...
- `fit_fixl_R<RRR>um.mat`：**fix_dir** `fit_KI_fixl` 在各取樣半徑 R 的解（`ell`/`gB`/`Khat`/`J`/`errpct`；R=150 → ell≈0.856 mm）。供 `Hall_sensor_base_fix_dir` 載入 ℓ̂。
...
- 產生：`bias_fit/.../calib_fem_bias.m`（→ `calib_bias.mat`）、`fixl_fit/.../fit_KI_full.m`（→ `fit_KI_full.mat`）、
  ...
  `Calibration_using_FEM_modeling/fix_dir/code/main/main.m`（→ `fit_fixl_R<RRR>um.mat`）。
- 消費：`Hall_sensor_base_no_fix_dir` + `no_fix_dir` plot（讀 `calib_bias.mat`）、
  ...
  `Hall_sensor_base_fix_dir/code/main/main.m`（讀 `fit_fixl_R150um.mat` 的 `ell`）。
