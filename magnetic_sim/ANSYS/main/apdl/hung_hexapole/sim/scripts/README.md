...
- `_build_gap100um_mu_eq.py` — 幾何不動，加 `MAT_PROT` μ_r=56、mesh 後把 protrusion 元素 reassign，產生 gap100um_mueq 6 支。⚠ 以「複製 `gap_200um` decks」為模板，但該 deck 已退場 → 此產生器**不可重跑**，僅留歷史（gap100 decks 已產；μ_eq sweep 改用 `mueq_sweep` 兩段式）。
