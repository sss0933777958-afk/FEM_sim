...
**用途**：這個 model（Long Fei 6 極 hexapole，下極 P1/P3/P6 磨平半錐、上極 P2/P4/P5 完整錐）所有 ANSYS MAPDL 腳本的 topic 根。涵蓋幾何建模 + IGES 匯出（`geom/`）、求解（`sim/`）、後處理抽場（`postproc/`）三大階段。是重跑 sim 的 input。

**內容**：本層只放三個子資料夾，無散落 .txt：
