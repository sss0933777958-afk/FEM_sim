...
- `scripts/` — 產生上述 sim .txt 的 `.py` 輔助（`_build_gap100um_mu_eq.py`、`_build_from_long2016_verbatim.py`、`_generate_halfcut_sims.py`）。⚠ `_build_gap100um_mu_eq.py` 以「複製 `gap_200um` decks」為模板，但該模板已退場 → 不可重跑，僅留作歷史（gap100 decks 已產）。
...
**資料來源 / 流向**：幾何取自 `hexapole-long2016` Long2016 source verbatim；解出 `.dat`（場）/ `.db`（模型）存到 `ANSYS_data/long2016_hexapole_halfcut/coilN/{standard,graded,gap_200um}/`。
