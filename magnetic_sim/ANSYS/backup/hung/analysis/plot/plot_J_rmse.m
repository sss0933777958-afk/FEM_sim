%% plot_J_rmse.m — [J] ideal K_I with RMSE error metric
...
for k=1:6
    ...
    res=bmod-bf;
    sq_err = res(1:Ns).^2 + res(Ns+1:2*Ns).^2 + res(2*Ns+1:3*Ns).^2;
    all_sq_err=[all_sq_err; sq_err];
end
rmse = sqrt(mean(all_sq_err)) * 1e3;  % mT
all_abs_err = sqrt(all_sq_err) * 1e3;  % |dB| per point [mT]
Ntotal=length(all_abs_err);
...
% (b) Absolute error scatter + RMSE line
...
print(fig,fullfile('..','..','figures','analytic','fitting_J_idealKI_RMSE.png'),'-dpng','-r200');
fprintf('Saved. RMSE = %.4f mT\n', rmse);
