...
## Description
Same parts as `magnetic_sim/hung/IGES/`, but with IGES unit flag corrected from 6 (mm) to 1 (inches).
This ensures reliable conversion when saving as STEP in SolidWorks or other CAD software.

## Difference from `magnetic_sim/hung/IGES/`
- `magnetic_sim/hung/IGES/` — raw ANSYS export, unit flag = 6 (mm), values in inches. SolidWorks opens correctly but STEP conversion may fail.
- `magnetic_sim/hung/IGES_converted/` — unit flag fixed to 1 (inches). SolidWorks opens correctly AND STEP conversion preserves dimensions.
...
## Regenerating
When parts change, re-export from `magnetic_sim/hung/IGES/` and fix unit flag:
```bash
